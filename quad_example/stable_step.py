...
def gradients(G, A, S, U, B):
    ...
    S_grad = - Sinv.T.dot(R.dot(G)-A).dot(S.T).dot(B.T).dot(U.T).dot(Sinv.T) + B.T.dot(U.T).dot(Sinv.T).dot(R.dot(G)-A)
...
def stabilize_discrete(G, A, S, U, B, max_iter=10):
    ...
    Kd = np.linalg.inv(Sn).dot(Un).dot(Bn).dot(Sn)
    return Sn, Un, Bn, Kd,0. 
